...
import deleteimg from './deleteimg.png';// with require;
...
function Child() {
    ...
    const handleSubmit = (event) => {
        ...
        addTransaction({ amount: Number(newAmount), desc: newDesc, id: uuidv4()})
    }

    const deleteTransactionbyDelete = (id) => {
console.log(id);
                ...
                {transactions.map((transaction, ind) => {
                    return (
                        <li key={ind}>
                            ...
                            <button onClick={()=>deleteTransactionbyDelete(transaction.id)}><img src={deleteimg} /></button>
                        </li>
                    )
                })}
            </ul>
